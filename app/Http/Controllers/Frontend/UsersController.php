...
class UsersController extends Controller
{
    ...
    public function store(StoreUserRequest $request)
    {
        ...
        if ($request->input('pic', false)) {
            $user->addMedia(storage_path('tmp/uploads/' . basename($request->input('pic'))))->toMediaCollection('pic');
        }
    ...
    public function update(UpdateUserRequest $request, User $user)
    {
        ...
        $user->roles()->sync($request->input('roles', [2]));
        if ($request->input('pic', false)) {
            if (!$user->pic || $request->input('pic') !== $user->pic->file_name) {
                if ($user->pic) {
                    $user->pic->delete();
                }
                $user->addMedia(storage_path('tmp/uploads/' . basename($request->input('pic'))))->toMediaCollection('pic');
