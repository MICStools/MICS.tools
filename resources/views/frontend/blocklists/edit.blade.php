        ...
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-header">
                    {{ trans('global.edit') }} {{ trans('cruds.blocklist.title_singular') }}
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.blocklists.update", [$blocklist->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="questions[]" id="questions" multiple required>
                                @foreach($questions as $id => $question)
                                    <option value="{{ $id }}" {{ (in_array($id, old('questions', [])) || $blocklist->questions->contains($id)) ? 'selected' : '' }}>{{ $question }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="answers[]" id="answers" multiple required>
                                @foreach($answers as $id => $answer)
                                    <option value="{{ $id }}" {{ (in_array($id, old('answers', [])) || $blocklist->answers->contains($id)) ? 'selected' : '' }}>{{ $answer }}</option>
                                @endforeach
                            </select>
