                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.answers.update", [$answer->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="number" name="order" id="order" value="{{ old('order', $answer->order) }}" step="1" required>
                            @if($errors->has('order'))
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="number" name="weight" id="weight" value="{{ old('weight', $answer->weight) }}" step="1" required>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="projects[]" id="projects" multiple>
                                @foreach($projects as $id => $project)
                                    <option value="{{ $id }}" {{ (in_array($id, old('projects', [])) || $answer->projects->contains($id)) ? 'selected' : '' }}>{{ $project }}</option>
                                @endforeach
                            </select>
