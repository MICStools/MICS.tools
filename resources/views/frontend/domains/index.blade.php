        ...
        <div class="col-md-12">
            @can('domain_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.domains.create') }}">
                            ...
                            <tbody>
                                @foreach($domains as $key => $domain)
                                    <tr data-entry-id="{{ $domain->id }}">
                                        ...
                                        <td>
                                            @can('domain_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.domains.show', $domain->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('domain_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.domains.edit', $domain->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('domain_delete')
                                                <form action="{{ route('frontend.domains.destroy', $domain->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('domain_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.domains.massDestroy') }}",
