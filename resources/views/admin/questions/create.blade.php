                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.questions.store") }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('help') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control ckeditor" name="help" id="help">{!! old('help') !!}</textarea>
                            @if($errors->has('help'))
                        ...
                        <div class="panel questionanswers">
                          <div class="panel-heading">
                            {{ trans('global.create') }} {{ trans('cruds.answer.title_singular') }}s
                          </div>
                          <ol>
                            ...
                            <li>
                              <div>
                                <input type="hidden" name="answers[{{ $i }}][order]" value="{{ $i }}">
                                <div class="form-group {{ $errors->has('text') ? 'has-error' : '' }}">
                                    <label for="answertext{{ $i }}">{{ trans('cruds.answer.fields.text') }}</label>
                                    <input class="form-control" type="text" name="answers[{{ $i }}][text]" id="answertext{{ $i }}" value="">
                                    @if($errors->has('text'))
                                        <span class="help-block" role="alert">{{ $errors->first('text') }}</span>
                                    @endif
                                ...
                                <div class="form-group {{ $errors->has('weight') ? 'has-error' : '' }}">
                                    <label for="answerweight{{ $i }}">{{ trans('cruds.answer.fields.weight') }}</label>
                                    <input class="form-control" type="number" name="answers[{{ $i }}][weight]" id="answerweight{{ $i }}" value="0" step="1">
                            ...
                            @if ($i < 19)
                              <input type="checkbox" id="answeradd{{ $i+1 }}">
                              <label class="btn btn-success" for="answeradd{{ $i+1 }}">Add answer</label>
                            @endif
                            @endfor
